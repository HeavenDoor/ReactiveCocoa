/*******************************************************************************
 # File        : ZXCommonDefinition.h
 # Project     : ZXBasePro
 # Author      : shenghai
 # Created     : 2018/03/29
 # Corporation : 四川蛙众科技有限公司
 # Description : 定义常用宏，可跨项目使用
 Description Logs
 -------------------------------------------------------------------------------
 # Date        : 2018/03/29
 # Author      : shenghai
...
#ifndef ZXCommonDefinition_h
#define ZXCommonDefinition_h
...
//整型转字符串【推荐使用】
#define ZXI2S(i) [NSString stringWithFormat:@"%ld",(long)(i)]
//浮点型转字符串【推荐使用】
#define ZXF2S(f) [NSString stringWithFormat:@"%f",(f)]
//浮点型转字符串，可以控制精度【推荐使用】
#define ZXF2S_P(f,p) [NSString stringWithFormat:@"%.#pf",(f)]
// int转NSString 【已废弃，不要再使用】
#define INT2STR(i) ZXI2S(i)
// float转NSString【已废弃，不要再使用】
#define FLOAT2STR(f) ZXF2S_P(f,4)
// 16进制字符串转换为数字(无符号长整型)
...
#define ZX_GLOBAL_ASYNC_BEGIN dispatch_async(dispatch_get_global_queue(0,0), ^{
#define ZX_GLOBAL_ASYNC_END   });

#define ZX_MAIN_ASYNC_BEGIN dispatch_async(dispatch_get_main_queue(), ^{
#define ZX_MAIN_ASYNC_END   });
